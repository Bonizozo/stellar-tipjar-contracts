...
fn test_borrow_with_sufficient_collateral() {
    ...
    let loan_id = loan::borrow(&env, pool_id, borrower.clone(), 1000, 1500).expect("borrow failed");
...
fn test_liquidate_undercollateralized_loan() {
    ...
    let loan_id = loan::borrow(&env, pool_id, borrower.clone(), 1000, 1100).expect("borrow failed");
